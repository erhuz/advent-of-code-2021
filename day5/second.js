...
const determineLineCrossings = (input) => {
    ...
    // Diagonals should start with their left most point,
    // and end with their right most point.
    for (let index = 0; index < lines.diagonal.length; index += 1) {
        const line = lines.diagonal[index];
        if(line[0][0] > line[1][0]){
            lines.diagonal[index] = line.reverse();
        }
    }
    ...
    for (let index = 0; index < lines.diagonal.length; index += 1) {
        const line = lines.diagonal[index];
        if(line[0][1] > line[1][1]){
            // If the line has an downwards angle.
            // Move downwards one step at a time and add each position to the vents object.

            const cursor = [...line[0]];
            while(true){
                addVentAt(cursor[0], cursor[1]);
                
                cursor[0] = cursor[0] + 1;
                cursor[1] = cursor[1] - 1;
                
                if(cursor[0] === line[1][0] && cursor[1] === line[1][1]){
                    addVentAt(cursor[0], cursor[1]);
                    break;
                }
            }
            ...
            // If the line has an upwards angle.
            // Move upwards one step at a time and add each position to the vents object.
